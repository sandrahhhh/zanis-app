...
class Products
{
	...
	public function getProducts(){
		$q = $this->con->query("SELECT p.product_id, p.status, p.product_title, p.product_price,p.product_qty, p.product_desc, p.product_image, p.product_keywords, c.cat_id, c.cat_title FROM products p JOIN categories c ON c.cat_id = p.product_cat");
	...
	public function addProduct(
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/Farmerspride/product_images/".$uniqueImageName)) {
					
					$q = $this->con->query("INSERT INTO `products`(`product_cat`,`status`, `product_title`, `product_qty`, `product_price`, `product_desc`, `product_image`, `product_keywords`) VALUES ('$category_id', '$status', '$product_name', '$product_qty', '$product_price', '$product_desc', '$uniqueImageName', '$product_keywords')");
